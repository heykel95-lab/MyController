...
struct Parameters {
  ...
  std::string csv_file_name = "general_axis_constraint_active_nullspace_return_log.csv";
...
int main() {
  try {
    ...
    std::cout << "Starting general axis-constraint impedance controller with active-task nullspace return, Coriolis switch and e+Enter stop." << std::endl;
    if (params.use_nullspace_optimization) {
      std::cout << "Active-task nullspace return is enabled: tau_active_nullspace is added to the task torque." << std::endl;
    }
    ...
    robot.control([&](const franka::RobotState& state,
      ...
      if (params.use_nullspace_optimization) {
        ...
        // Example:
        // fix_p = [1 0 0], fix_R = [1 1 0]
        // -> J_active contains rows x, Rx, Ry only.
        ...
        Eigen::Matrix<double, 7, 1> tau_joint_return =
            params.nullspace_k_start * (q_start_nullspace - q_current)
            - params.nullspace_damping * dq;

        // Project the joint return torque into the nullspace of the active task.
        // This strongly reduces arbitrary configuration changes while keeping
        // the fixed task axes mostly unaffected.
        tau_nullspace = N * tau_joint_return;
