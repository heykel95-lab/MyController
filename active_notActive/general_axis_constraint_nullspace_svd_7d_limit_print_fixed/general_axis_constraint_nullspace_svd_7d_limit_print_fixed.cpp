...
struct Parameters {
  ...
  std::string csv_file_name = "general_axis_constraint_nullspace_svd_7d_limit_print_fixed_log.csv";
...
int main() {
  try {
    ...
    if (params.use_nullspace_optimization) {
      std::cout << "Nullspace torque term is enabled: tau_nullspace is added to the task torque." << std::endl;
    }
    ...
    robot.control([&](const franka::RobotState& state,
      ...
      if (params.use_nullspace_optimization) {
        ...
        if (n.norm() > 1e-9) {
          ...
          // Return to the initial joint configuration, but only along
          // the current nullspace direction.
