/*
  Extended Impedance with Asymmetric Friction Compensation and Coriolis Switch
  -------------------------------------------------------------------------
...
struct Parameters {
  ...
  std::string csv_file_name = "general_axis_constraint_nullspace_sigma_only_no_limits_switch_log.csv";

  // Test switch:
  // use_coriolis = true  -> tau_cmd = J^T F + coriolis
  // use_coriolis = false -> tau_cmd = J^T F
  ...
  // This term tries to keep q near q_start and to increase sigma_min(J).
...
double nullspace_damping = 1.0;
  ...
  bool disable_controller_side_limits = false;// Initial joint configuration used before starting impedance control.
...
p.nullspace_damping = getDouble("nullspace_damping", p.nullspace_damping);
  ...
  p.disable_controller_side_limits = getBool("disable_controller_side_limits", p.disable_controller_side_limits);p.q_goal[0] = getDouble("q_goal_1", p.q_goal[0]);
...
int main() {
  std::cout << "NO-LIMITS SWITCH VERSION: disable_controller_side_limits is controlled in parameters.txt." << std::endl;
  try {
    ...
    if (params.use_coriolis && params.use_nullspace_optimization) {
      std::cout << "Torque command mode: tau_cmd = J^T * F + tau_nullspace + coriolis" << std::endl;
      ...
      std::cout << "Torque command mode: tau_cmd = J^T * F + tau_nullspace  (NO coriolis)" << std::endl;
    } else {
...
std::cout << "nullspace_damping: " << params.nullspace_damping << std::endl;
    ...
    if (params.disable_controller_side_limits) {
      std::cout << "Controller-side f_max, m_max and delta_tau_max limiters are DISABLED for this test." << std::endl;
      std::cout << "Franka internal safety/reflex/collision limits remain active." << std::endl;
    }
std::cout << "e_thresh_p [m]: " << params.e_thresh_p.transpose() << std::endl;
    ...
    if (params.axis_constraint_mode) {
      ...
      std::cout << "Translation: fixed axes sigma-only nullspace; free axes follow current position." << std::endl;
    ...
    std::cout << "Starting general axis-constraint impedance controller with nullspace sigma-min optimization, Coriolis switch and e+Enter stop." << std::endl;
    if (params.use_nullspace_optimization) {
      std::cout << "Nullspace sigma-min term is enabled: tau_nullspace is added to the task torque." << std::endl;
    }
    ...
    robot.control([&](const franka::RobotState& state,
      ...
      if (params.use_nullspace_optimization) {
        ...
        // SVD of the current 6x7 Jacobian.
        // For a 7-DOF robot, the last right singular vector is the
        // one-dimensional nullspace direction.
        Eigen::JacobiSVD<Eigen::Matrix<double, 6, 7>> svd_current(
            J, Eigen::ComputeFullU | Eigen::ComputeFullV);
        ...
        if (n.norm() > 1e-9) {
          ...
          Eigen::Map<const Eigen::Matrix<double, 6, 7>>
              J_plus(J_plus_array.data());

          Eigen::Map<const Eigen::Matrix<double, 6, 7>>
              J_minus(J_minus_array.data());
          ...
          // Direction that locally increases the smallest singular value.
          Eigen::Matrix<double, 7, 1> delta_q_sigma =
              sigma_direction * params.nullspace_alpha * n;

          // Damping only in the nullspace direction.
          Eigen::Matrix<double, 7, 1> dq_null =
              n.dot(dq) * n;

          tau_nullspace =
              params.nullspace_k_sigma * delta_q_sigma
              - params.nullspace_damping * dq_null;
