/*
  Simplified Impedance / Compliance Test with 6-Axis Friction Compensation
  -------------------------------------------------------------------------
  ...
  New in this version:
  - Friction-compensation force can be applied in all 3 positional axes.
  - Friction-compensation moment can be applied in all 3 rotational axes.
...
struct Parameters {
  ...
  std::string csv_file_name = "extended_impedance_friction_no_wrench_torque_limits_log.csv";
  ...
  Eigen::Vector3d f_fric = Eigen::Vector3d(0.5, 0.5, 0.5);

  Eigen::Vector3d e_thresh_R = Eigen::Vector3d(0.03, 0.03, 0.03);
  Eigen::Vector3d m_fric = Eigen::Vector3d(0.05, 0.05, 0.05);
};
...
Parameters readParameters(const std::string& filename) {
  ...
  p.f_fric(0) = getDouble("f_fric_x", p.f_fric(0));
  p.f_fric(1) = getDouble("f_fric_y", p.f_fric(1));
  p.f_fric(2) = getDouble("f_fric_z", p.f_fric(2));
  ...
  p.m_fric(0) = getDouble("m_fric_x", p.m_fric(0));
  p.m_fric(1) = getDouble("m_fric_y", p.m_fric(1));
  p.m_fric(2) = getDouble("m_fric_z", p.m_fric(2));
...
// Add a small friction-compensation component in all selected axes.
// For position, this returns a force.
// For rotation, this returns a moment.
// The signs follow the error vector direction because f = K e already uses
// the error direction as restoring direction.
Eigen::Vector3d addMinimumRestoringComponent(
    const Eigen::Vector3d& command,
    const Eigen::Vector3d& error,
    const Eigen::Vector3d& threshold,
    const Eigen::Vector3d& friction_command) {

  Eigen::Vector3d result = command;

  for (int i = 0; i < 3; ++i) {
    if (friction_command(i) <= 0.0) {
      continue;
    }

    if (std::abs(error(i)) > threshold(i)) {
      result(i) += friction_command(i) * ((error(i) > 0.0) ? 1.0 : -1.0);
    }
  }
...
int main() {
  try {
    ...
    std::cout << "delta_tau_max [Nm/cycle]: " << params.delta_tau_max
              << "  (not used in this no-limits variant)" << std::endl;
    std::cout << "WARNING: This variant does not apply f_max, m_max, or torque-rate limiting." << std::endl;
    std::cout << "e_thresh_p [m]: " << params.e_thresh_p.transpose() << std::endl;
    std::cout << "f_fric [N]: " << params.f_fric.transpose() << std::endl;
    std::cout << "e_thresh_R [rad]: " << params.e_thresh_R.transpose() << std::endl;
    std::cout << "m_fric [Nm]: " << params.m_fric.transpose() << std::endl;
    ...
    std::cout << "Starting extended impedance controller WITHOUT f_max/m_max/delta_tau_max limits." << std::endl;
    ...
    robot.control([&](const franka::RobotState& state,
      ...
      f = addMinimumRestoringComponent(
          f, e_p, params.e_thresh_p, params.f_fric);

      m = addMinimumRestoringComponent(
          m, e_R, params.e_thresh_R, params.m_fric);
      // No f_max saturation in this variant.
      // No m_max saturation in this variant.
Eigen::Matrix<double, 6, 1> wrench;
      ...
      // No joint torque-rate limiting in this variant.
    ...
    std::cerr << "If the robot is still in an error/reflex state, recover it manually in Franka Desk. This no-limits variant may be more aggressive." << std::endl;
    return -1;
