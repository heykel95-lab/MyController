...
struct Parameters {
  ...
  std::string csv_file_name = "virtual_yz_plane_impedance_log.csv";
...
int main() {
  try {
    ...
    if (params.virtual_yz_plane_mode) {
      std::cout << "Virtual Y-Z plane active: x is constrained, y and z are allowed/damped." << std::endl;
    }
    ...
    std::cout << "Starting virtual Y-Z plane impedance controller." << std::endl;
    ...
    robot.control([&](const franka::RobotState& state,
      ...
      if (params.virtual_yz_plane_mode) {
        // Virtual Y-Z plane:
        // keep x at the start value, but allow y and z to follow the current position.
        // This removes the position spring in y and z, while x remains constrained.
        p_d(0) = p_start(0);
        p_d(1) = p_EE(1);
        p_d(2) = p_EE(2);

        // Desired velocity in y/z is kept at zero, so y/z still have viscous damping.
        // If completely free y/z motion is desired, set Dp_y = Dp_z = 0 in parameters.txt.
        pdot_d(0) = 0.0;
        pdot_d(1) = 0.0;
        pdot_d(2) = 0.0;
      }
