/*
  Friend-like Simple Cartesian Impedance Controller
  ------------------------------------------------
  Purpose:
  This file is intentionally kept close to the simple compliance/impedance structure
  used in the reference/friend code.
  ...
  It does NOT include:
      ...
      - friction compensation f_fric / m_fric
      - rho(e) shaping
      - trajectory mode

  Why:
  This makes it easier to compare your safer controller with the simpler
  friend-like controller.
  ...
  At the end it prints:
      ...
      - final raw torque command
      - max position error during experiment
      - max rotation error during experiment

  CSV:
  The file also logs the experiment to CSV.
*/
...
struct Parameters {
  ...
  std::string csv_file_name = "friend_like_simple_impedance_log.csv";
...
Eigen::Vector3d orientationError(
  ...
  // Express orientation error in base frame.
...
int main() {
  try {
    ...
    std::cout << "Friend-like simple impedance controller" << std::endl;
    ...
    std::cout << "Starting friend-like simple impedance controller." << std::endl;
    ...
    robot.control([&](const franka::RobotState& state,
      ...
      // Friend-like simple impedance law.
      // Desired velocity is zero because this is a static holding test.
    ...
    std::cerr << "This friend-like controller has no f_max, m_max, torque-rate limit, or friction compensation." << std::endl;
    std::cerr << "If it stops by reflex, compare that with your safer controller." << std::endl;
    return -1;
