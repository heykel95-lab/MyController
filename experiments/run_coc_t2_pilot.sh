#!/usr/bin/env bash
# Guided one-repeat t2-axis centre-of-compliance sign pilot.
#
#   ./experiments/run_coc_t2_pilot.sh status
...
RUN_IDS=(
  PILOT_COC_t2_rc_t1_m040
  PILOT_COC_t2_rc_t1_p040
)
...
case "${1:-status}" in
  ...
  next)
    if ! run_id="$(find_next)"; then
      echo "The t2-axis centre-of-compliance sign pilots are complete."
      exit 0
    fi
