#!/usr/bin/env python3
"""Create the active controller plane from three measured probe points.

The first three rows define the plane. Any later rows are held out and reported
as validation distances. Coordinates are base-frame metres at the calibrated
physical probe point, not uncorrected end-effector positions.

Usage:
  cp experiments/calibration/plane_points.example.csv \
...
POINTS = os.path.join(HERE, "plane_points.csv")
OVERLAY = os.path.join(HERE, "active_plane_overlay.txt")
REPORT = os.path.join(HERE, "plane_calibration_report.txt")
...
def load_points(path):
    ...
    if len(points) < 3:
        sys.exit("At least three non-collinear probe points are required.")
    return points
...
def main():
    if not os.path.exists(POINTS):
        sys.exit(
            f"Missing {POINTS}\n"
            "Copy plane_points.example.csv to plane_points.csv and enter the "
            "measured base-frame probe coordinates."
        )

    labelled = load_points(POINTS)
    ...
    if validation:
        ...
        report_lines.append(
            "No held-out point supplied. Add P4 to check the fitted plane."
        )

    with open(OVERLAY, "w") as handle:
        handle.write("# Generated by prepare_plane_calibration.py.\n")
        handle.write("# Applied before every D-series experiment overlay.\n")
