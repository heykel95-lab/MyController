...
The stronger follow-on keeps the original +100 mm Case-F records unchanged.
It applies 40 N at a +200 mm link-3 point, raises the disturbance-torque
ceiling from 2.0 to 5.0 Nm, and tests sigma torque only at
1.5 and 2.0 Nm. Run its mode-0 hardware gate first:

```bash
./experiments/run.sh PILOT_F_disturbance_40N_200mm 1
python3 experiments/analysis/validate_nullspace_pilot.py \
  experiments/results/PILOT_F_disturbance_40N_200mm/r01
./experiments/run_axis_study.sh case F auto
```

The unattended follow-on remains blocked unless the pilot delivers the full
40 N waveform without torque clipping, exceeds 4.0 Nm equivalent joint-torque
