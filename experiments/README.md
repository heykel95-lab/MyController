...
It moves the virtual link-3 point to +150 mm on the local z axis, raises the
disturbance-torque ceiling from 2.0 to 2.5 Nm, and tests sigma torque only at
1.5 and 2.0 Nm. Run its mode-0 hardware gate first:

```bash
./experiments/run.sh PILOT_F_disturbance_20N_150mm 1
python3 experiments/analysis/validate_nullspace_pilot.py \
  experiments/results/PILOT_F_disturbance_20N_150mm/r01
./experiments/run_axis_study.sh case F auto
```
