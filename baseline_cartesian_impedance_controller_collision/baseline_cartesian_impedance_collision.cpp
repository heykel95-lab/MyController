...
struct Parameters {
  ...
  std::string csv_file_name = "baseline_cartesian_impedance_collision_log.csv";
  ...
  // Optional Franka collision/contact thresholds.
  // These are robot-side thresholds and are included so the baseline and
  // extended controller can be compared under the same robot safety setting.
  bool use_custom_collision_behavior = true;

  std::array<double, 7> collision_torque_lower_acc = {{30.0, 30.0, 30.0, 30.0, 30.0, 30.0, 30.0}};
  std::array<double, 7> collision_torque_upper_acc = {{30.0, 30.0, 30.0, 30.0, 30.0, 30.0, 30.0}};
  std::array<double, 7> collision_torque_lower_nom = {{30.0, 30.0, 30.0, 30.0, 30.0, 30.0, 30.0}};
  std::array<double, 7> collision_torque_upper_nom = {{30.0, 30.0, 30.0, 30.0, 30.0, 30.0, 30.0}};
...
Parameters readParameters(const std::string& filename) {
  ...
  const double collision_joint_torque =
      getDouble("collision_joint_torque_threshold", 30.0);
  for (int i = 0; i < 7; ++i) {
  ...
  const double collision_cart_force =
      getDouble("collision_cartesian_force_threshold", 30.0);
  for (int i = 0; i < 3; ++i) {
  ...
  const double collision_cart_moment =
      getDouble("collision_cartesian_moment_threshold", 20.0);
  for (int i = 3; i < 6; ++i) {
...
int main() {
  try {
    ...
    std::cout << "use_custom_collision_behavior: "
              << params.use_custom_collision_behavior << std::endl;
    if (params.use_custom_collision_behavior) {
      std::cout << "collision joint torque threshold [Nm]: "
                << params.collision_torque_lower_acc[0] << std::endl;
      std::cout << "collision Cartesian force threshold [N]: "
                << params.collision_force_lower_acc[0] << std::endl;
      std::cout << "collision Cartesian moment threshold [Nm]: "
                << params.collision_force_lower_acc[3] << std::endl;
    }
