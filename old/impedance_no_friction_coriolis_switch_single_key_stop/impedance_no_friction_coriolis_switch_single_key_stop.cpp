...
struct Parameters {
  ...
  std::string csv_file_name = "extended_impedance_asymmetric_friction_coriolis_switch_log.csv";
...
int main() {
  try {
    ...
    if (params.experiment_duration <= 0.0) {
      std::cout << "Time mode: indefinite run, stop by pressing e." << std::endl;
    } else {
      std::cout << "Time mode: automatic stop after experiment_duration, or earlier by pressing e." << std::endl;
    }
    ...
    std::cout << "Starting impedance controller WITHOUT friction compensation, with Coriolis switch and single-key stop." << std::endl;
    ...
    std::cout << "Press e during the run to stop the control loop safely." << std::endl;
    if (params.experiment_duration <= 0.0) {
      std::cout << "experiment_duration <= 0: running indefinitely until you press e." << std::endl;
    } else {
      std::cout << "Otherwise, the experiment stops automatically after experiment_duration, or earlier if you press e." << std::endl;
    }

    std::thread keyboard_thread([&stop_requested, &control_finished]() {
      while (!control_finished.load()) {
        if (keyboardStopRequested()) {
          stop_requested.store(true);
          std::cout << std::endl << "Keyboard stop requested with key e. Finishing control loop..." << std::endl;
          break;
        }
      }
    });
    ...
    robot.control([&](const franka::RobotState& state,
      ...
      Eigen::Matrix<double, 7, 1> tau_raw = tau_task;
      if (params.use_coriolis) {
        tau_raw += coriolis;
      }
