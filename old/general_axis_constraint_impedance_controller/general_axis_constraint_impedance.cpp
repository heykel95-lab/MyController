...
struct Parameters {
  ...
  std::string csv_file_name = "general_axis_constraint_impedance_log.csv";

  bool use_current_pose = true;

  // General axis constraint mode:
  // For translation:
  //   fix_p_i = true  -> p_d_i = p_start_i, axis is constrained
  ...
  // For rotation:
  // The rotational error vector e_R is computed as usual.
  // Then each component can be kept or released:
  //   fix_R_i = true  -> keep rotational stiffness in that component
  //   fix_R_i = false -> set e_R_i = 0, no rotational spring in that component
  //
  // Damping still depends on DR_i. For a fully free rotational axis, also set DR_i=0.
  bool fix_R_x = true;
  bool fix_R_y = true;
  bool fix_R_z = true;
...
int main() {
  try {
    ...
    std::cout << "Starting general axis-constraint impedance controller." << std::endl;
    ...
    robot.control([&](const franka::RobotState& state,
      ...
      if (params.axis_constraint_mode) {
        ...
        // free axis: desired position follows the current position
        // This creates a virtual plane/line/point depending on which axes are fixed.
        ...
        // For a completely free axis, set the corresponding Dp value to 0.
        pdot_d(0) = 0.0;
        pdot_d(1) = 0.0;
        pdot_d(2) = 0.0;
      }
