/*
  Simplified Impedance / Compliance Test with Semi-Automatic Recovery
  ------------------------------------------------------------------
  This version follows the working project structure and adds a recovery step.
  ...
  2. CSV logging is buffered.
     Data are stored in memory during the real-time callback and written
  ...
  4. Safe first-test mode:
     ...
     position and orientation are used as the desired pose.

  5. Semi-automatic recovery:
     Before setDefaultBehavior(robot), the program asks the user to confirm
     that the workspace is clear and then calls:
         robot.automaticErrorRecovery();
     This can recover the robot after a previous reflex stop without going
     manually to Franka Desk every time.

  Important safety note:
  - Automatic error recovery should only be attempted after checking that
    the workspace is clear and the emergency stop is reachable.
*/
...
struct Parameters {
  ...
  std::string csv_file_name = "simplified_impedance_fixed_recovery_log.csv";
...
int main() {
  try {
    ...
    // ============================================================
    // Semi-automatic recovery before setDefaultBehavior
    // ============================================================
    ...
    // Apply standard behavior after recovery.
    ...
    robot.control([&](const franka::RobotState& state,
      ...
      // Add Coriolis compensation only.
