...
int main() {
  try {
    ...
    if (params.experiment_duration <= 0.0) {
      std::cout << "Time mode: indefinite run, stop with q + Enter." << std::endl;
    } else {
      std::cout << "Time mode: automatic stop after experiment_duration, or earlier with q + Enter." << std::endl;
    }
    ...
    std::cout << "Starting impedance controller WITHOUT friction compensation, with Coriolis switch and robust keyboard stop." << std::endl;
    ...
    std::cout << "Type q and press Enter during the run to stop the control loop safely." << std::endl;
    if (params.experiment_duration <= 0.0) {
      std::cout << "experiment_duration <= 0: running indefinitely until q + Enter." << std::endl;
    } else {
      std::cout << "Otherwise, the experiment stops automatically after experiment_duration." << std::endl;
    }
    ...
    std::thread keyboard_thread([&stop_requested, &control_finished]() {
      while (!control_finished.load()) {
        if (keyboardStopRequested()) {
          ...
          std::cout << std::endl << "Keyboard stop requested. Finishing control loop..." << std::endl;
          break;
        }
      }
    });
