/*
  ...
  Final command:
      tau_cmd = tau_task + coriolis + gravity

  Gravity compensation is added, following the working project.
...
struct Parameters {
  ...
  std::string csv_file_name = "extended_impedance_gravity_compensation_log.csv";
...
int main() {
  try {
    ...
    std::cout << "Gravity compensation: enabled, tau_cmd = tau_task + coriolis + gravity" << std::endl;
    ...
    std::cout << "Starting 6-axis impedance controller with gravity compensation." << std::endl;
    ...
    robot.control([&](const franka::RobotState& state,
      ...
      // This term was not included in the Coriolis-only version.
      std::array<double, 7> gravity_array =
          model.gravity(state);

      Eigen::Map<const Eigen::Matrix<double, 7, 1>>
          gravity(gravity_array.data());

      Eigen::Matrix<double, 7, 1> tau_raw =
          tau_task + coriolis + gravity;
