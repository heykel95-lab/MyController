...
struct Parameters {
  ...
  std::string csv_file_name = "general_axis_constraint_nullspace_svd_one_enter_clean_log.csv";
  ...
  // Uses the 1D nullspace of the 6x7 Jacobian.
  // Goal 1: keep joints close to the start configuration q_start.
  // Goal 2: choose the nullspace direction that increases sigma_min(J).
  bool use_nullspace_optimization = true;
  double nullspace_k_start = 3.0;
...
int main() {
  try {
    ...
    if (params.use_coriolis) {
      ...
      std::cout << "Torque command mode: tau_cmd = J^T * F  (NO coriolis)" << std::endl;
    }
    ...
    std::cout << "nullspace_k_start: " << params.nullspace_k_start << std::endl;
...
Moving to the initial joint configuration after the single startup confirmation..." << std::endl;
    ...
    Eigen::Map<const Eigen::Matrix<double, 7, 1>>
        q_start_nullspace(initial_state.q.data());
    ...
    std::cout << "Starting general axis-constraint impedance controller with nullspace SVD optimization, Coriolis switch and e+Enter stop." << std::endl;
    ...
    robot.control([&](const franka::RobotState& state,
      ...
      if (params.use_nullspace_optimization) {
        ...
        // Singular value decomposition of the current 6x7 Jacobian.
        // The last right singular vector is the 1D nullspace direction.
        ...
        if (n.norm() > 1e-9) {
          ...
          // Compare two small nullspace moves q +/- alpha*n.
          ...
          // Part 1: return to the start joint configuration, but only
          // along the current nullspace direction.
          const double a_start =
              n.dot(q_start_nullspace - q_current);

          Eigen::Matrix<double, 7, 1> delta_q_start =
              a_start * n;

          // Part 2: move in the nullspace direction that increases sigma_min.
          Eigen::Matrix<double, 7, 1> delta_q_sigma =
              sigma_direction * params.nullspace_alpha * n;

          // Damping only along the nullspace direction.
          Eigen::Matrix<double, 7, 1> dq_null =
              n.dot(dq) * n;

          tau_nullspace =
              params.nullspace_k_start * delta_q_start
              + params.nullspace_k_sigma * delta_q_sigma
              - params.nullspace_damping * dq_null;

          tau_nullspace = limitVectorNorm(
              tau_nullspace, params.nullspace_tau_max);
        }
      }
      ...
      Eigen::Matrix<double, 7, 1> tau_raw = tau_task + tau_nullspace;
      if (params.use_coriolis) {
        tau_raw += coriolis;
      }
