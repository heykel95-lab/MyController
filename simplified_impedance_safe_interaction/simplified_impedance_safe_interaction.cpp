/*
  Simplified Impedance / Compliance Test with Safe Interaction Limits
  ------------------------------------------------------------------
  Added for safer manual compliance tests:
  - Cartesian force saturation.
  - Cartesian moment saturation.
  - Joint torque-rate limiting.
  - Semi-automatic robot recovery.
  - Buffered CSV logging.

  The torque command follows the working project:
      tau_cmd = tau_task + coriolis

  Gravity compensation is not added.
*/
...
struct Parameters {
  ...
  std::string csv_file_name = "simplified_impedance_safe_interaction_log.csv";
...
double smoothStepDerivative(double r, double T) {
  ...
  const double ds_dr = 30.0 * std::pow(r, 2) - 60.0 * std::pow(r, 3) + 30.0 * std::pow(r, 4);
  return ds_dr / T;
}

Eigen::Vector3d orientationError(const Eigen::Matrix3d& R_current,
...
void writeLogToCsv(const std::vector<LogData>& log_data,
  ...
  log_file << "time,"
           ...
           << "tau_limited_1,tau_limited_2,tau_limited_3,tau_limited_4,tau_limited_5,tau_limited_6,tau_limited_7"
           << "\n";
...
int main() {
  try {
    ...
    Eigen::Map<const Eigen::Matrix<double, 4, 4>> T_initial(initial_state.O_T_EE.data());
    ...
    robot.control([&](const franka::RobotState& state,
      ...
      Eigen::Matrix<double, 7, 1> tau_raw = tau_task + coriolis;
